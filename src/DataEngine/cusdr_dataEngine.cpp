	...
	, m_sMeterCalibrationOffset(0.0f)//(35.0f)
{
	...
	rxDisplayList = set->getRxDisplayList();
	m_lastRxIndex = 0;
}
...
DataEngine::~DataEngine() {
	...
	if (m_AudioThread->isRunning()) {
		...
		DATA_ENGINE_DEBUG << "Audio thread stopped and deleted.";
	}

	if (m_dataIO && m_dataIOThread->isRunning()) {
		m_dataIO->networkDeviceStartStop(0);
	}

	if (!m_dspThreadList.empty()) {
...
void DataEngine::initAudioEngine() {
	...
	if (m_AudioThread->isRunning()) {
		...
		DATA_ENGINE_DEBUG << "audio thread started.";
		...
		DATA_ENGINE_DEBUG << "could not start audio thread !";
		//setSystemState(QSDR::DataProcessThreadError, m_hwInterface, m_serverMode, QSDR::DataEngineDown);
		//return false;
	}

	//m_audioEngine->play("LittleMartha.mp3");
	//m_audioEngine->play("Eureka.m4a");
	//m_audioEngine->play("0db_60Chirps_Phil.wav");
}
...
void DataEngine::setupConnections() {
	...
	CHECKED_CONNECT(
		set, 
		SIGNAL(numberOfRXChanged(QObject *, int, int, const QList<bool> &)), 
		this, 
		SLOT(setNumberOfRx(QObject *, int, int, const QList<bool> &)));
...
bool DataEngine::startDataEngineWithoutConnection() {
	...
	if (io.inputBuffer.length() > 0) {
		...
		//initReceivers(1);
		if (RX.count() == 0) { // first time start: create receivers

			if (!addReceiver(0)) {
				
				DATA_ENGINE_DEBUG << "error creating Rx 0";
				return false;
			}
		}
...
bool DataEngine::findHPSDRDevices() {
	...
	if (m_hpsdrDevices == 0) {
		...
		QList<TNetworkDevicecard> metisList = set->getHPSDRDeviceList();
		...
		io.hpsdrDeviceIPAddress = set->getCurrentHPSDRDevice().ip_address;
		io.hpsdrDeviceName = set->getCurrentHPSDRDevice().boardName;
...
bool DataEngine::getFirmwareVersions() {
	...
	// init receivers
	m_lastRxIndex = rxDisplayList.lastIndexOf(true) + 1;

	QString str = "Initializing %1 receiver(s)...please wait";
	set->setSystemMessage(str.arg(rxDisplayList.count(true)), m_lastRxIndex * 500);
		
	if (RX.count() < m_lastRxIndex) { // first time start: create receivers

		for (int i = RX.count(); i < m_lastRxIndex; i++) {

			if (!addReceiver(i)) {
				
				DATA_ENGINE_DEBUG << "error creating Rx " << i;
				return false;
			}
		}
	}
...
bool DataEngine::start() {
	...
	if (RX.count() < m_lastRxIndex) {

		DATA_ENGINE_DEBUG << "error: m_lastRxIndex != RX.count().";
		return false;
	}
	...
	for (int i = 0; i < m_lastRxIndex; i++) {
		...
		if (rxDisplayList.at(i)) {
			...
			if (m_dspThreadList.at(i)->isRunning()) {
				...
				//setSystemState(QSDR::DataProcessThreadError, m_hwInterface, m_serverMode, QSDR::DataEngineDown);
				//return false;
				DATA_ENGINE_DEBUG << "receiver processor thread for Rx " << i << " not started!";
			}
		}
	}
	...
	// IQ data processing thread
	if (!startDataProcessor(QThread::NormalPriority)) { //QThread::HighPriority
...
void DataEngine::stop() {
	...
	if (m_dataEngineState == QSDR::DataEngineUp) {
		...
		DATA_ENGINE_DEBUG << "receiver threads stopped.";
...
bool DataEngine::addReceiver(int rx) {

	Receiver* rcvr = new Receiver(rx);

	// init the DSP core
	DATA_ENGINE_DEBUG << "trying to init a DSP core for rx " << rx;

	if (rcvr->initDSPInterface()) {

		DATA_ENGINE_DEBUG << "init DSP core for rx " << rx << " successful !";

		rcvr->setConnectedStatus(false);
		rcvr->setServerMode(m_serverMode);

		// create dsp thread
		QThreadEx* thread = new QThreadEx();
		rcvr->moveToThread(thread);
...
void DataEngine::initIOData() {

	m_txFrame = 0;
	
	//io.currentReceiver = 0;
	io.receivers = rxDisplayList.count(true);
	io.maxReceiverNo = rxDisplayList.lastIndexOf(true) + 1;
	...
	io.control_out[4] = (io.ccTx.duplex << 2) | ((io.maxReceiverNo - 1) << 3);
}
...
void DataEngine::setNumberOfRx(QObject *sender, int numberOfRx, int lastChanged, const QList<bool> &list) {

	Q_UNUSED (sender)
	Q_UNUSED (lastChanged)

	if (io.receivers == numberOfRx) return;

	rxDisplayList = list;

	//m_lastRxIndex = rxDisplayList.lastIndexOf(true) + 1;

	if (m_serverMode == QSDR::SDRMode && m_dataEngineState == QSDR::DataEngineUp) {
		...
		// add/remove receiver
		int newLastRxIndex = rxDisplayList.lastIndexOf(true) + 1;
		
		if (newLastRxIndex == m_lastRxIndex + 1) { // add a receiver
		
			if (RX.count() < newLastRxIndex) {
		
				addReceiver(newLastRxIndex-1);
				set->setCurrentReceiver(this, newLastRxIndex-1);
			}
		}
		else if (newLastRxIndex == m_lastRxIndex - 1) { // remove one receiver from RX and his thread

			if (RX.at(newLastRxIndex)) RX.removeLast();

			if (m_dspThreadList.at(newLastRxIndex)) m_dspThreadList.removeLast();

			set->setCurrentReceiver(this, 0);
		}

		m_lastRxIndex = newLastRxIndex;
		...
		if (!m_wbDataProcessor) createWideBandDataProcessor();
		...
		//set->setCurrentReceiver(this, set->getCurrentReceiver());
		set->setSystemMessage("Data Engine restarted.", 4000);
	}
	else {
	...
	DATA_ENGINE_DEBUG << "number of receivers set to " << io.receivers;
}
...
void DataEngine::loadWavFile(const QString &fileName) {
	...
	if (m_audioEngine->loadAudioFile(fileName))
	...
	, m_stopped(false)
{
	...
	m_deviceAddress = set->getCurrentHPSDRDevice().ip_address;
}
...
void DataProcessor::processDeviceData() {
	...
	forever {
		...
		//DATA_PROCESSOR_DEBUG << "iq_queue empty? " << de->io.iq_queue.isEmpty();
...
void DataProcessor::processInputBuffer(const QByteArray &buffer) {
	...
	if (buffer.at(s++) == SYNC && buffer.at(s++) == SYNC && buffer.at(s++) == SYNC)	{
        ...
        while (s < m_maxSamples) {
            ...
            for (int r = 0; r < de->io.maxReceiverNo; r++) {
			...
			if (m_fwCount < 100)
			{
				if (m_hwInterface == QSDR::Metis)
				{
					...
					if (de->io.ccRx.devices.hermesFWVersion != buffer.at(4)) {
						...
						de->io.networkIOMutex.lock();
						DATA_ENGINE_DEBUG << "firmware version: " << qPrintable(QString::number(buffer.at(4)));
						de->io.networkIOMutex.unlock();
					}
				}
				m_fwCount++;
...
void DataProcessor::encodeCCBytes() {
    ...
    switch (m_sendState) {
    	...
    	case 0:
			...
			de->io.control_out[4] |= (de->io.maxReceiverNo - 1) << 3;
...
void WideBandDataProcessor::processWideBandInputBuffer(const QByteArray &buffer) {
	...
	if ((io->mercuryFW > 32 || io->hermesFW > 11) && set->getModel() != QSDR::anan10E)
