...
{	
	...
	//qDebug() << "trying to play something...";

	//m_audioThread->play("LittleMartha.mp3");
	//m_audioThread->play("Eureka.m4a");
}
...
void MainWindow::setupConnections() {
	...
	CHECKED_CONNECT(
		...
		this, 
		SLOT(setNumberOfReceivers(QObject *, int, int, const QList<bool> &)));
...
void MainWindow::setup() {
	...
	createReceiverPanels(set->getMaxNumberOfReceivers());
	
	// the dock windows event filter
	m_dockEventFilter = new DockEventFilter();
...
void MainWindow::setupLayout() {
	...
	for (int i = 1; i < set->getMaxNumberOfReceivers(); i++) {
		...
		dock->installEventFilter(m_dockEventFilter);
	...
	CHECKED_CONNECT(
			m_dockEventFilter, 
			SIGNAL(receiverDockClosed(int)), 
			this, 
			SLOT(receiverDockClosed(int)));

	// the outline of the receiver panels
	if (set->getMaxNumberOfReceivers() > 5) {

		for (int i = 0; i < (int)(set->getMaxNumberOfReceivers() - 1)/2; i++) {
...
void MainWindow::receiverDockVisibilityChanged(bool value) {

	QDockWidget* dock = qobject_cast<QDockWidget* >(sender());
	int rx = rxDockWidgetList.indexOf(dock);

	
	QList<bool> rxList = set->getRxDisplayList();
	MAIN_DEBUG << "rxList: " << rxList;
	//if (rxList.at(rx+2)) return;

	MAIN_DEBUG << "receiver " << rx+2 << " dock visible: " << value;

	if (!value) {
		set->removeReceiver(rx+1);
	}
}

void MainWindow::receiverDockClosed(int rx) {

	QList<bool> rxList = set->getRxDisplayList();
	MAIN_DEBUG << "rxList: " << rxList;
	MAIN_DEBUG << "closing rx " << rx;
...
//	firstBtnLayout->setMargin(0);
	...
	hpsdrBtn = new AeroButton("HW Config", this);
	...
	addRxBtn = new AeroButton("Add Rx", this);
	addRxBtn->setRoundness(10);
    addRxBtn->setFont(m_fonts.normalFont);
    addRxBtn->setTextColor(btnCol);
	addRxBtn->setFixedSize(btn_width1, btn_height1);
...
//		SLOT(getLastFrequency()));
	...
	firstBtnLayout->addWidget(displayBtn);
	firstBtnLayout->addWidget(nullBtn);
	firstBtnLayout->addWidget(addRxBtn);
	firstBtnLayout->addWidget(modeBtn);
	firstBtnLayout->addWidget(quitBtn);
...
void MainWindow::setNumberOfReceivers(
		...
		int last,					/*!<[in] the last receiver which was added/removed. */
		const QList<bool> &rxlist	/*!<[in] the list of opened receiver displays. */
) {
	Q_UNUSED (sender)
	Q_UNUSED (value)
	Q_UNUSED (last)

	for (int i = 0; i < set->getMaxNumberOfReceivers() - 1; i++) {
...
void MainWindow::setChirpWSPRMode(bool value) {
	...
	setNumberOfReceivers(this, 1, 0, rxList);
...
void MainWindow::getNetworkInterfaces() {
	...
	MAIN_DEBUG << "last HPSDR device ip address: " << qPrintable(set->getLastHPSDRDevice().ip_address.toString()) << " .";
}
	...
	,	set(Settings::instance())
{
	...
	m_deviceCards = set->getHPSDRDeviceList();
...
//***************************************************************************
// Receiver dock window Event filter class
...
bool DockEventFilter::eventFilter(QObject* obj, QEvent* event) {
	...
	if (event->type() == QEvent::Close) {
		...
		int rx = str.toInt();
		qDebug() << "DockEventFilter:\tclose event received for rx " << rx;

		emit receiverDockClosed(rx);
