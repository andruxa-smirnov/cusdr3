	...
	, m_currentReceiver(0)
{
	...
	m_versionString = "v0.3.3.0  (Qt 5.4.0)";
...
int Settings::loadSettings() {
	...
	str = settings->value("network/last_device", "127.0.0.1").toString();
	while (str.startsWith('\"')) str = str.right(str.count() - 1).trimmed();
	while (str.endsWith('\"')) str = str.left(str.count() - 1).trimmed();
	m_lastHPSDRDevice.ip_address = QHostAddress(str);
	...
	// SDR hardware
	//value = settings->value("hw/max_receivers", 4).toInt();
	//if (value < 0 || value > 7) value = 4;
	//m_maxReceivers = value;

	// init receiver data list
	initReceiverList();
	
	value = settings->value("hw/hardware", 0).toInt();
	if (value < 0 || value > 2) value = 0;
	m_hpsdrHardware = value;

	// hw type (Atlas, Hermes, Anan,..)
	str = settings->value("hw/type", "atlas").toString();
	if (str == "atlas") {
		...
		m_model = QSDR::anan100;
		m_hwInterface = QSDR::Hermes;

		m_maxReceivers = 7;
	}
	else if (str == "anan100D") {

		m_model = QSDR::anan100D;
		m_hwInterface = QSDR::Hermes;

		m_maxReceivers = 7;
	}
	else if (str == "anan200D") {

		m_model = QSDR::anan200D;
		m_hwInterface = QSDR::Hermes;
	...
	//str = settings->value("hw/hermes", "false").toString();
	//if (str == "true")
	...
	//	str = settings->value("hw/interface", "metis").toString();
	//		if (str == "metis")
	//			m_hwInterface = QSDR::Metis;
	//		else
	//			m_hwInterface = QSDR::NoInterfaceMode;
	...
	str = settings->value("hw/checkfw", "true").toString();
	if (str == "true")
...
int Settings::saveSettings() {
	...
	settings->setValue("network/last_device", m_lastHPSDRDevice.ip_address.toString());
	...
	// hardware
	settings->setValue("hw/max_receivers", m_maxReceivers);
	settings->setValue("hw/hardware", m_hpsdrHardware);

	// hw type (Atlas, Hermes, Anan,..)
	if (m_model == QSDR::atlas)
		settings->setValue("hw/type", "atlas");
	else if (m_model == QSDR::anan10)
		settings->setValue("hw/type", "anan10");
	else if (m_model == QSDR::anan10E)
		settings->setValue("hw/type", "anan10E");
	else if (m_model == QSDR::anan100)
		settings->setValue("hw/type", "anan100");
	else if (m_model == QSDR::anan100D)
		settings->setValue("hw/type", "anan100D");
	else if (m_model == QSDR::anan200D)
		settings->setValue("hw/type", "anan200D");
	else
		settings->setValue("hw/type", "noModel");
	...
	switch (m_hpsdrHardware) {
		...
		case 0:
			...
			if (m_devices.mercuryPresence)
				settings->setValue("hw/mercury", "true");
			else
				settings->setValue("hw/mercury", "false");

			if (m_devices.penelopePresence)
				settings->setValue("hw/penelope", "true");
			else
				settings->setValue("hw/penelope", "false");

			if (m_devices.pennylanePresence)
				settings->setValue("hw/pennylane", "true");
			else
				settings->setValue("hw/pennylane", "false");

			if (m_devices.excaliburPresence)
				settings->setValue("hw/excalibur", "true");
			else
				settings->setValue("hw/excalibur", "false");
			break;
	...
	if (m_devices.alexPresence)
		settings->setValue("hw/alex", "true");
	else
		settings->setValue("hw/alex", "false");


//	if (m_devices.hermesPresence)
//		settings->setValue("hw/hermes", "true");
//	else
//		settings->setValue("hw/hermes", "false");
	...
	switch (m_hpsdrHardware) {
		...
		case 0:
			...
			if (m_hwInterface == QSDR::Metis)
				settings->setValue("hw/interface", "metis");
			else if (m_hwInterface == QSDR::NoInterfaceMode)
				settings->setValue("hw/interface", "noInterface");
			break;
		...
		case 1:
			...
			if (m_hwInterface == QSDR::Hermes)
				settings->setValue("hw/interface", "hermes");
			break;
	...
	if (m_checkFirmwareVersions)
		settings->setValue("hw/checkfw", "true");
	else
		settings->setValue("hw/checkfw", "false");
...
void Settings::initReceiverList() {
	...
	for (int i = 0; i < m_maxReceivers; i++) {
		...
		// fill the receiver list; set the first receiver true
		if (i == 0) {
			m_rxDisplayList << true;
		}
		else {
			m_rxDisplayList << false;
		}

		//emit rxDisplayListChanged(this, m_rxDisplayList);
	}
}
...
void Settings::debugSystemState() {
	...
	SETTINGS_DEBUG << "SDR Board:\t\t" << qPrintable(getModelString(m_model));
...
void Settings::setModel(QSDR::_SDRModel model) {

	if (m_model == model) return;
	m_model = model;

	SETTINGS_DEBUG << "selected board: " << qPrintable(getModelString(m_model));
	emit modelChanged(this, m_model);
}

QSDR::_SDRModel	Settings::getModel() {
...
QString	Settings::getModelString(QSDR::_SDRModel model) {
...
void Settings::setHPSDRDeviceList(QList<TNetworkDevicecard> list) {
	...
	locker.unlock();
	emit metisCardListChanged(m_HPSDRDevices);
}
...
void Settings::clearHPSDRDeviceList() {

	m_HPSDRDevices.clear();
...
void Settings::setMaxNumberOfReceivers(int value) {

	if (value > MAX_RECEIVERS)
		m_maxReceivers = MAX_RECEIVERS;
	else
		m_maxReceivers = value;
}

void Settings::addReceiver() {

	//QMutexLocker locker(&settingsMutex);

	SETTINGS_DEBUG << "current number of receivers: " << m_receivers;
	if (m_receivers + 1 > m_maxReceivers) {

		SETTINGS_DEBUG << "maximal number of receivers reached.";
		setSystemMessage("Maximal number of receivers reached.", 4000);
		return;
	}
	else {
		...
		int index = m_rxDisplayList.indexOf(false);
		if (index < 0) {
			SETTINGS_DEBUG << "error m_rxDisplayList.";
		}

		m_rxDisplayList[index] = true;
		SETTINGS_DEBUG << m_rxDisplayList;
		SETTINGS_DEBUG << "active receivers: " << m_rxDisplayList.count(true);
		SETTINGS_DEBUG << "m_receivers: " << m_receivers;

		if (m_receivers != m_rxDisplayList.count(true)) {

			SETTINGS_DEBUG << "error: number of receivers unequals m_rxDisplayList length!";
			return;
		}
		emit numberOfRXChanged(this, m_receivers, index, m_rxDisplayList);
		
		//setCurrentReceiver(this, index);
		//setCurrentReceiver(this, 0);
	}
}

void Settings::removeReceiver(int value) {

	if (value == 0 || m_receivers < 2) return;

	//QMutexLocker locker(&settingsMutex);

	m_receivers--;

	m_rxDisplayList[value] = false;
	SETTINGS_DEBUG << m_rxDisplayList;
	SETTINGS_DEBUG << "active receivers: " << m_rxDisplayList.count(true);
	SETTINGS_DEBUG << "m_receivers: " << m_receivers;

	emit numberOfRXChanged(this, m_receivers, value, m_rxDisplayList);
}
...
void Settings::setdBmPanScaleMin(int rx, qreal value) {
	...
	//SETTINGS_DEBUG << "setdBmPanScaleMin: " << m_receiverDataList[rx].dBmPanScaleMinList[band];
...
void Settings::setdBmPanScaleMax(int rx, qreal value) {
	...
	//SETTINGS_DEBUG << "setdBmPanScaleMax: " << m_receiverDataList[rx].dBmPanScaleMaxList[band];
