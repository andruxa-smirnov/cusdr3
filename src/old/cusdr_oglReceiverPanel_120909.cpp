...
void QGLReceiverPanel::drawPanadapter() {
	...
	if (m_dataEngineState == QSDR::DataEngineUp) {
		...
		if (m_adcStatus == 2) {
			...
			QFontMetrics fmHuge(m_hugeFont);
			...
			int rectWidth = fmHuge.boundingRect(str).width();
...
void QGLReceiverPanel::drawCrossHair() {
	...
	QFontMetrics fmSmall(m_smallFont);
	QFontMetrics fmBig(m_bigFont1);
	...
	if (m_smallSize) {
		...
		rectWidth = fmSmall.boundingRect(fstr).width();
		fontHeight = fmSmall.tightBoundingRect("0").height() + spacing;
	}
	else {

		rectWidth = fmBig.width(fstr);
		fontHeight = fmBig.tightBoundingRect("0").height() + spacing;
	}
...
void QGLReceiverPanel::drawReceiverInfo() {
	...
	if (m_receiver == set->getCurrentReceiver())
		...
		glColor3f(0.54f, 0.42f, 0.13f);

	QString str;
	if (set->getNumberOfReceivers() > 1) {
	...
	if (m_panRect.height() > 15) {
		...
		/*if (f1 / 1000 < 10) {
			...
			qglColor(QColor(0, 0, 0, alpha));
			m_oglTextFreq2->renderText(x + 36, 22, 4.0f, str.arg(f2, 3, 10, QLatin1Char('0')));
			m_oglTextFreq1->renderText(x + 60, 22, 4.0f, "MHz");

			qglColor(QColor(255, 255, 255, alpha));
			m_oglTextFreq2->renderText(x + 34, 20, 5.0f, str.arg(f2, 3, 10, QLatin1Char('0')));
			m_oglTextFreq1->renderText(x + 58, 20, 5.0f, "MHz");
		}
		else {

			qglColor(QColor(0, 0, 0, alpha));
			m_oglTextFreq2->renderText(x + 39, 22, 4.0f, str.arg(f2, 3, 10, QLatin1Char('0')));
			m_oglTextFreq1->renderText(x + 65, 22, 4.0f, "MHz");

			qglColor(QColor(255, 255, 255, alpha));
			m_oglTextFreq2->renderText(x + 41, 20, 5.0f, str.arg(f2, 3, 10, QLatin1Char('0')));
			m_oglTextFreq1->renderText(x + 63, 20, 5.0f, "MHz");
		}*/
	}
...
void QGLReceiverPanel::renderPanVerticalScale() {
	...
	if (m_smallSize) {
		...
		fontHeight = fmSmall.tightBoundingRect(".0dBm").height() + spacing;
		fontMaxWidth = fmSmall.boundingRect("-000.0").width();
	}
	else {

		fontHeight = fmBig.tightBoundingRect(".0dBm").height() + spacing;
		fontMaxWidth = fmBig.boundingRect("-000.0").width();
	}
	...
	if (len > 0) {
		...
		for (int i = 0; i < len; i++) {
			...
			if (textRect.y() >= yOld && textRect.bottom() <= (m_dBmScalePanRect.height() - textRect.height())) {
				...
				if (m_smallSize)
					m_oglTextSmall->renderText(textRect.x() + fontMaxWidth - fmSmall.tightBoundingRect(str).width(), textRect.y(), str);
				else
					m_oglTextBig1->renderText(textRect.x() + fontMaxWidth - fmSmall.tightBoundingRect(str).width(), textRect.y(), str);
...
void QGLReceiverPanel::renderPanHorizontalScale() {
	...
	if (m_smallSize) {
		...
		fontHeight = fmSmall.tightBoundingRect(".0kMGHz").height();
		fontMaxWidth = fmSmall.boundingRect("000.000.0").width();
	}
	else {

		fontHeight = fmBig.tightBoundingRect(".0kMGHz").height();
		fontMaxWidth = fmBig.boundingRect("000.000.0").width();
	}
	...
	if (m_smallSize)
		scaledTextRect.setWidth(fmSmall.width(fstr));
	else
		scaledTextRect.setWidth(fmBig.width(fstr));
	...
	if (len > 0) {
		...
		for (int i = 0; i < len; i++) {
			...
			int textWidth;
			if (m_smallSize)
				textWidth = fmSmall.width(str);
			else
				textWidth = fmBig.width(str);
...
void QGLReceiverPanel::wheelEvent(QWheelEvent* event) {
	...
	double freqStep = set->getMouseWheelFreqStep();
