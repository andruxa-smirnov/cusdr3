...
//	m_oglTextSmall->renderText(textRect.x(), textRect.y(), str);
	...
	if (x > m_panRect.width() - m_haircrossMaxRight) {
		...
		if (y > m_haircrossMinTop) {
			...
			//showText(tx, ty, 5.0f, str, m_smallSize);
			if (m_smallSize)
				m_oglTextSmall->renderText(x - m_haircrossOffsetLeft, y - 42, 5.0f, str);
			else
				m_oglTextBig1->renderText(x - m_haircrossOffsetLeft, y - 42, 5.0f, str);
		}
		else {
			...
			if (m_smallSize)
				...
				m_oglTextBig1->renderText(x - m_haircrossOffsetLeft, y + 26, 5.0f, str);
		}
	}
	else {
		...
		if (y > m_haircrossMinTop) {
			...
			if (m_smallSize)
				...
				m_oglTextBig1->renderText(x + m_haircrossOffsetRight, y - 42, 5.0f, str);
		}
		else {
			...
			if (m_smallSize)
				...
				m_oglTextBig1->renderText(x + m_haircrossOffsetRight, y + 26, 5.0f, str);
		}
	}
	...
	if (x > m_panRect.width() - m_haircrossMaxRight) {
		...
		if (y > m_haircrossMinTop)
			m_oglTextSmall->renderText(x - m_haircrossOffsetLeft, y + 6, 5.0, m_bandText);
		else
			m_oglTextSmall->renderText(x - m_haircrossOffsetLeft, y + 54, 5.0, m_bandText);
	}
	else {

		if (y > m_haircrossMinTop)
			m_oglTextSmall->renderText(x + m_haircrossOffsetRight, y + 6, 5.0, m_bandText);
		else
			m_oglTextSmall->renderText(x + m_haircrossOffsetRight, y + 54, 5.0, m_bandText);
	}
	...
	if (m_mouseRegion == panadapterRegion) {
		...
		if (x > m_panRect.width() - m_haircrossMaxRight) {
			...
			if (y > m_haircrossMinTop)
				m_oglTextSmall->renderText(x - m_haircrossOffsetLeft, y - 18, 5.0, str);
			else
				m_oglTextSmall->renderText(x - m_haircrossOffsetLeft, y + 30, 5.0, str);
		}
		else {

			if (y > m_haircrossMinTop)
				m_oglTextSmall->renderText(x + m_haircrossOffsetRight, y - 18, 5.0, str);
			else
				m_oglTextSmall->renderText(x + m_haircrossOffsetRight, y + 30, 5.0, str);
		}
	}
...
void QGLReceiverPanel::setupDisplayRegions(QSize size) {
	...
	if (m_panRect.height() > m_bigHeight && m_panRect.width() > m_bigWidth)
		m_smallSize = false;
	else
		m_smallSize = true;
