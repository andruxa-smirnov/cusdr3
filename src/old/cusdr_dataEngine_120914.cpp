	...
	if (io.inputBuffer.length() > 0) {
		...
		switch (m_serverMode) {
			...
			case QSDR::ChirpWSPRFile:			
				...
				io.cpxIn.resize(2*BUFFER_SIZE);
				io.cpxOut.resize(2*BUFFER_SIZE);
	...
	if ((m_serverMode == QSDR::ChirpWSPR) && !m_chirpProcessor)
		...
		switch (m_serverMode) {
			...
			case QSDR::DttSP:
				...
				//foreach(Receiver *r, RX)
				//	r->initDttSPInterface();
...
void DataEngine::initReceivers() {
	...
	//iodata.current_receiver = set->getCurrentReceivers();
...
void DataEngine::initDttSPInterface(int rx) {
	...
	setDttSPMainVolume(this, rx, RX.at(rx)->getAudioVolume());
	//setDttspDspMode(this, rx, rxList.at(rx)->getDSPMode());
	DSPMode mode = RX.at(rx)->getDSPModeList().at(RX.at(rx)->getHamBand());
	setDttspDspMode(this, rx, mode);
	setDttspAgcMode(this, rx, RX.at(rx)->getAGCMode());
	setDttspAgcGain(this, rx, RX.at(rx)->getAGCGain());
...
void DataEngine::initDSPEngine(int rx) {
	...
	setQtDSPMainVolume(this, rx, RX.at(rx)->getAudioVolume());
        ...
        switch (io.ccRx.roundRobin) { // cycle through C0
			...
			case 0:
				...
				if (io.ccRx.lt2208) { // check ADC signal
					...
					if (m_ADCChangedTime.elapsed() > 100) {
...
void DataEngine::dttspProcessing() {
	...
	for (int r = 0; r < io.receivers; r++) {
		...
		if (RX.at(r)->getConnectedStatus()) {
			...
			dttSPList.at(r)->dttspProcessPanadapter(0, RX.at(r)->spectrum);



			//if (r == io.currentReceiver)
			//	dttSPList.at(r)->dttspProcessSpectrum(0, RX.at(r)->postSpectrum);
			...
			if (r == io.currentReceiver) {
				...
				//processOutputBuffer(RX.at(r)->in, &RX.at(r)->in[BUFFER_SIZE]);
			}
		}
	}
}
...
void DataEngine::qtdspProcessing() {
	...
	for (int r = 0; r < io.receivers; r++) {
		...
		if (RX.at(r)->getConnectedStatus()) {
			...
			// do the funky DSP
			qtdspList.at(r)->processDSP(RX.at(r)->inBuf, RX.at(r)->outBuf, BUFFER_SIZE);

			// spectrum
			qtdspList.at(r)->getSpectrum(RX.at(r)->spectrum);

			//if (rxList.at(r)->highResTimer->getElapsedTimeInMicroSec() >= m_fpsList.at(r)) {
			if (RX.at(r)->highResTimer->getElapsedTimeInMicroSec() >= RX.at(r)->getDisplayDelay()) {
...
void DataEngine::dttSPDspProcessing(int rx) {
	...
	//if (rx == io.currentReceiver)
		dttSPList[rx]->dttspAudioCallback(
				RX[rx]->in, &RX[rx]->in[BUFFER_SIZE],
				RX[rx]->out, &RX[rx]->out[BUFFER_SIZE], BUFFER_SIZE, 0);

	// process spectrum data (pre filter)
	//io.mutex.lock();
	//if (rx == io.currentReceiver)
		//dttSPList[rx]->dttspProcessPanadapter(0, RX[rx]->spectrum);
		dttSPList[rx]->dttspProcessPanadapter(0, RX[rx]->temp);
	//io.mutex.unlock();
	
	// process spectrum data (post filter)
	/*io.mutex.lock();
		dttSPList[rx]->dttspProcessSpectrum(0, RX[rx]->spectrum);
	io.mutex.unlock();*/
	...
	//if (rx == io.currentReceiver) {
		for (int i = 0; i < 4 * BUFFER_SIZE; i++) {
			...
			RX[rx]->spectrum[i] = RX.at(rx)->temp[i] * 0.1 + RX.at(rx)->spectrum[i] * 0.9;
		}
		//emit spectrumBufferChanged(rxList[rx]->spectrum);
		//settings->setSpectrumBuffer(rxList.at(rx)->spectrum);
		settings->setSpectrumBuffer(rx, RX.at(rx)->spectrum);
	//}

	if (rx == io.currentReceiver)
		//processOutputBuffer(io.out_buffer, &io.out_buffer[BUFFER_SIZE]);
		processOutputBuffer(RX[rx]->out, &RX[rx]->out[BUFFER_SIZE]);
}
