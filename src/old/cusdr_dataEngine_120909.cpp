...
void DataEngine::initReceivers() {
	...
	for (int i = 0; i < settings->getNumberOfReceivers(); i++) {
		...
		// set all receiver data from settings.ini
		rcvr->setReceiverData(settings->getReceiverDataList().at(i));
	...
	io.ccTx.currentBand = rxList.at(0)->getHamBand();
	io.ccTx.mercuryAttenuators = rxList.at(0)->getMercuryAttenuators();
	io.ccTx.mercuryAttenuator = rxList.at(0)->getMercuryAttenuators().at(io.ccTx.currentBand);
	...
	//io.ccTx.alexConfiguration = settings->getAlexConfiguration();
	...
	//setAlexConfiguration(io.ccTx.alexConfiguration);
...
void DataEngine::initDttSPInterface(int rx) {
	...
	DSPMode mode = rxList.at(rx)->getDSPModeList().at(rxList.at(rx)->getHamBand());
...
void DataEngine::dttspProcessing() {
	...
	for (int r = 0; r < io.receivers; r++) {
		...
		if (rxList.at(r)->getConnectedStatus()) {
			...
			/*if (m_spectrumTimer.elapsed() >= m_framesCnt) {
...
void DataEngine::setSampleRate(QObject *sender, int value) {
	...
	{	
		if (dttSPList[io.currentReceiver]->getDttSPStatus()) {
			...
			HamBand band = rxList.at(io.currentReceiver)->getHamBand();
			DSPMode mode = rxList.at(io.currentReceiver)->getDSPModeList().at(band);
