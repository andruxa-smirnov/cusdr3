	...
	, m_currentReceiver(0)
{
	...
	memcpy(w.data(), y.data(), sizeof(cpx) * 2);

	qDebug() << "w1 = (" << w.at(0).re << "," << w.at(0).im << ")";
	qDebug() << "w2 = (" << w.at(1).re << "," << w.at(1).im << ")";
	*/
}
...
QString Settings::getDisplayToolbarStyle()	{ return displayToolbarStyle; }
QString Settings::getMainBtnToolbarStyle()	{ return mainBtnToolbarStyle; }
...
QString Settings::getDoubleSpinBoxStyle()	{ return dSpinBoxStyle; }
...
//		m_receiverDataList[i].waterfallTime = value;
		...
		if (m_receiverDataList[i].dBmPanScaleMinList.length() == MAX_BANDS &&
			...
			for (int j = 0; j < MAX_BANDS; j++) {
				...
				m_receiverDataList[i].dBmPanScaleMinList[j] = (qreal)(1.0 * value);
				...
				value = settings->value(cstr, -140).toInt();
				if ((value < -200) || (value > 0)) value = -140;
				m_receiverDataList[i].dBmPanScaleMaxList[j] = (qreal)(1.0 * value);
			}
		}
	}
	...
	color = settings->value("colors/gridLine", QColor(100, 110, 130)).value<QColor>();
	if (!color.isValid()) color = QColor(100, 110, 130);
